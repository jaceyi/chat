...
exports.sendMessage = functions.database.ref('/messages/{messageId}').onCreate(async (snapshot, context) => {
  const { messageId } = context.params;
  try {
    const tokenSnapshot = await admin.database().ref('messageTokens').once('value');
